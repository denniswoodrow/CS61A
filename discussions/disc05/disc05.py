...
def find_path(tree, x):
	"""Return a list that contains the nodes along the path from root to node containing x.
...
def prune_binary(t, nums):
	"""Return a tree that contains the numbers in nums that exist in t.
	>>> t = tree('1', [tree('0', [tree('0'), tree('1')]), tree('1', [tree('0')])])
	>>> prune_binary(t, ['01', '110', '100'])
	tree('1', [tree('0', [tree('0')]), tree('1', [tree('0')])])
	"""
	if is_leaf(t):
		if label(t) in nums:
			return t
