...
def closer_city(lat, lon, city_a, city_b):
    ...
    closer = min(city_a, city_b, key=lambda city: distance(home, city))
...
def riffle(deck):
    ...
    "*** YOUR CODE HERE ***"
    middle = len(deck)//2
    return sum([[deck[x], deck[middle + x]] if x % 2 == 0 
        else [deck[x], deck[middle + x]] 
...
def add_trees(t1, t2):
    ...
    for i in range(min(len(t1_branches), len(t2_branches))):
        b1, b2 = t1_branches[i], t2_branches[i]
